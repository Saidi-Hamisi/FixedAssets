...
export class AssetCrudService {
 ...
 // private baseUrl = 'http://52.15.152.26:9096/api/assets'; 
  ...
  createAsset(data: any): Observable<any> {
    const createAssetUrl = `${environment.baseGetUrl}/assets/addAsset`;
    return this.http.post<any>(createAssetUrl, data);
  }
...
//  Get select options
  ...
  getImageBarCode(id: number): Observable<Blob> {
    return this.http.get(`${environment.baseUrl}`+`/${id}/barcode`, { responseType: 'blob' });
  }
